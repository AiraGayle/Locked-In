...
export const useTimerState = (members, user) => {
  return useMemo(() => {
    ...
    if (!currentUserMember || !currentUserMember.session_id) {
      return { targetSeconds: 25 * 60, secondsLeft: 25 * 60, isRunning: false, hasStarted: false, originalTargetSeconds: 25 * 60 };
    }
    ...
    if (isOngoing && currentUserMember.startedAt) {
      ...
      return { targetSeconds, secondsLeft: remaining, isRunning: true, hasStarted: true, originalTargetSeconds };
    }

    if (isPaused) {
      const remaining = Number(currentUserMember.remainingSeconds) || targetSeconds;
      return { targetSeconds, secondsLeft: remaining, isRunning: false, hasStarted: true, originalTargetSeconds };
    }

    return { targetSeconds, secondsLeft: targetSeconds, isRunning: false, hasStarted: false, originalTargetSeconds };
  }, [members, user.id]);
};